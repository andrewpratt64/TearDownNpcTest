...
-- Returns true if npc was initialized correctly
function initNpc(npc)
	-- Get npc body
	npc.body = FindBody("npc", true);
	if (IsHandleValid(npc.body) == false) then return false; end
	npc.bodyT = GetBodyTransform(npc.body);

	-- Get npc shapes
	npc.shapes = GetBodyShapes(npc.body);
	if (#npc.shapes == 0) then return false; end
	
	-- Get head
	for _, shape in pairs(npc.shapes) do
		if (HasTag(shape, "npc_head")) then
			npc.headShape = shape;
			npc.headBody = GetShapeBody(npc.headBody);
			if (IsHandleValid(npc.headBody) == false) then return false; end
			npc.headBodyT = GetBodyTransform(npc.headBody);
			break;
		end
	end
end
